...
class RobotStateDisplay(cozmo.annotate.Annotator):
    def apply(self, image, scale):
        ...
        def print_line(text_line):
            text = cozmo.annotate.ImageText(text_line, position=cozmo.annotate.TOP_LEFT, outline_color='black',
                                            color='lightblue')
...
class RemoteControlCozmo:
    ...
    def handle_key(self, key_code, is_shift_down, is_ctrl_down, is_alt_down, is_key_down):
        ...
        '''
        if key_code == 13 and is_key_down:
    ...
    def update_mouse_driving(self):
        ...
        forward_speed = self.pick_speed(300, 150, 75)
        turn_speed = self.pick_speed(200, 100, 30)
...
def handle_index_page():
    return render_template('index.html', mouse=to_js_bool_string(_is_mouse_look_enabled_by_default),
                           debug=str(_display_debug_annotations))
