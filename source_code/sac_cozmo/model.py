...
class QNetworkCNN(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim):
        ...
        self.linear1 = nn.Linear(
            conv2d_size_out(IMAGE_H, conv) * conv2d_size_out(IMAGE_W, conv) * conv['conv3'][1] + num_actions,
            hidden_dim)
        self.linear2 = nn.Linear(hidden_dim, hidden_dim)
        self.linear3 = nn.Linear(hidden_dim, 1)
...
class GaussianPolicyCNN(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim):
        ...
        self.linear1 = nn.Linear(conv2d_size_out(IMAGE_H, conv) * conv2d_size_out(IMAGE_W, conv) * conv['conv3'][1],
                                 hidden_dim)
        self.linear2 = nn.Linear(hidden_dim, hidden_dim)
