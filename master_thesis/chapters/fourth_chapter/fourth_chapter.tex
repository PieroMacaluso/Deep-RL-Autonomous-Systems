...
\begin{figure}
    ...
    \caption[Interaction Human/Robot]{ Interaction between the user and Anki Cozmo.
        ...
        The last component consists of TensorBoard thanks to the script can store results that can be retrieved later by the user.}
    \label{fig:system}
\end{figure}
...
\begin{itemize}
    \item \textbf{Episode Phase.} This phase consists of a single episode take.
          ...
          In the early part of the experiment, the algorithm has to perform a warm-up, a procedure where the agent makes random actions to randomly explore the environment and obtain an initial set of entries for experience replay memory.
          After the agent gathered the minimum amount of entries, which is usually equal to the batch size or a multiple thereof, the agent starts to exploit the neural network to act using a noisy policy for exploration sake.
          This last-mentioned part is still an episode phase, but it is never followed by a testing or a checkpoint phase.
    ...
    \item \textbf{Undo Phase.} As described in \vref{subsec:error-management}, it is possible to invalidate experiments due to a human or a system error. For this reason, we implemented a procedure that the user can trigger to delete the knowledge acquired in the last training episode and restore the previous state. We positioned it right after the episode phase to delete faulty episodes as soon as possible.
          ...
          In this case, the user has two possibilities to undo the last episode. The first is to hit the backspace button in substitution of the second enter button hit we cited in the previous point: this choice is the fasted one and it restore the state before the last episode without passing through train, test or checkpoint phases. The second one leads to a delayed reset by hitting the backspace button after the second enter button hit and before the next one: in this scenario, the system has to pass through train, test and checkpoint phase and only afterwards it restores the previous episode.
    \item \textbf{Training Phase.} This phase starts to be executed after the initial warm-up phase, and it is located after each Cozmo episode. It manages all neural networks updates with several optimisation steps, also known as epochs: the number of optimisation steps is set as a parameter by the developer.
    ...
    The user can also trigger this phase manually by toggling C button before starting the next episode by hitting enter button. It is really useful to virtually pause experiments, like the one with Cozmo, that could last tens of hours and that can need multiple days to complete.
\end{itemize}
...
\begin{figure}
    ...
    \begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
        ...
        \draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (261.5,118) -- (396.5,118) -- (396.5,158) -- (261.5,158) -- cycle ;
        %Straight Lines [id:da45650982537203766] 
        \draw    (329.75,85) -- (329.75,115.5) ;
        \draw [shift={(329.75,118.5)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Shape: Diamond [id:dp7100269867943787] 
        \draw  [fill={rgb, 255:red, 248; green, 231; blue, 28 }  ,fill opacity=1 ] (330.38,181.5) -- (396.25,208.75) -- (330.38,236) -- (264.5,208.75) -- cycle ;
        %Straight Lines [id:da33363937943735045] 
        \draw    (163.5,393.75) -- (255.5,393.75) ;
        \draw [shift={(258.5,393.75)}, rotate = 180] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Straight Lines [id:da08256872780772095] 
        \draw    (163.5,353) -- (163.5,393.75) ;


        %Shape: Rectangle [id:dp6141481346237292] 
        \draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (101.5,117) -- (236.5,117) -- (236.5,157) -- (101.5,157) -- cycle ;
        %Straight Lines [id:da4102417822212999] 
        \draw    (255.5,136) -- (237.5,136) ;

        \draw [shift={(258.5,136)}, rotate = 180] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Straight Lines [id:da9410796039373476] 
        \draw    (330.38,158) -- (330.38,178.5) ;
        \draw [shift={(330.38,181.5)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Shape: Diamond [id:dp23419639551228588] 
        \draw  [fill={rgb, 255:red, 248; green, 231; blue, 28 }  ,fill opacity=1 ] (330.38,259.5) -- (403.5,290.25) -- (330.38,321) -- (257.25,290.25) -- cycle ;
        %Straight Lines [id:da4002053530831412] 
        \draw    (330.38,236) -- (330.38,256.5) ;
        \draw [shift={(330.38,259.5)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Straight Lines [id:da1961250189231556] 
        \draw    (163.5,289.75) -- (258.5,289.75) ;


        %Straight Lines [id:da08341935205642348] 
        \draw    (163.5,309) -- (163.5,289.75) ;

        \draw [shift={(163.5,312)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Shape: Rectangle [id:dp8998227827849266] 
        \draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (101.5,312) -- (236.5,312) -- (236.5,352) -- (101.5,352) -- cycle ;
        %Straight Lines [id:da03747681592112351] 
        \draw    (330.38,320) -- (330.38,360) ;
        \draw [shift={(330.38,363)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Shape: Diamond [id:dp05801551745223743] 
        \draw  [fill={rgb, 255:red, 248; green, 231; blue, 28 }  ,fill opacity=1 ] (330.38,362.5) -- (403.5,393.25) -- (330.38,424) -- (257.25,393.25) -- cycle ;
        %Straight Lines [id:da13095592205091366] 
        \draw    (169.5,208.75) -- (264.5,208.75) ;


        %Straight Lines [id:da6941565717302706] 
        \draw    (169.5,160) -- (169.5,208.75) ;

        \draw [shift={(169.5,157)}, rotate = 90] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Straight Lines [id:da30876512932326383] 
        \draw    (440.5,497.75) -- (330.38,497.75) ;


        %Straight Lines [id:da37637290320114314] 
        \draw    (505.5,457) -- (505.5,471) ;
        \draw [shift={(505.5,474)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;

        %Straight Lines [id:da6802339671267228] 
        \draw    (505.5,393.75) -- (403.5,393.75) ;


        %Straight Lines [id:da04855194568912746] 
        \draw    (505.5,413) -- (505.5,393.75) ;

        \draw [shift={(505.5,416)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Shape: Rectangle [id:dp5249336746554131] 
        \draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (443.5,416) -- (578.5,416) -- (578.5,456) -- (443.5,456) -- cycle ;
        %Rounded Rect [id:dp5401419126637466] 
        \draw  [fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (261.5,50) .. controls (261.5,45.58) and (265.08,42) .. (269.5,42) -- (388.5,42) .. controls (392.92,42) and (396.5,45.58) .. (396.5,50) -- (396.5,74) .. controls (396.5,78.42) and (392.92,82) .. (388.5,82) -- (269.5,82) .. controls (265.08,82) and (261.5,78.42) .. (261.5,74) -- cycle ;
        %Straight Lines [id:da46969392222154227] 
        \draw    (330.38,424) -- (330.38,497.75) ;


        %Straight Lines [id:da01473953482461099] 
        \draw    (330.38,497.75) -- (330.38,531.5) ;


        %Straight Lines [id:da47014594978326474] 
        \draw    (330.38,531.5) -- (596.5,531.5) ;


        %Straight Lines [id:da42252207031576927] 
        \draw    (596.5,140) -- (596.5,531.5) ;


        %Straight Lines [id:da5491155657054972] 
        \draw    (401.5,140) -- (596.5,140) ;

        \draw [shift={(398.5,140)}, rotate = 0] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
        %Shape: Rectangle [id:dp8147683219865862] 
        \draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (443.5,476) -- (578.5,476) -- (578.5,516) -- (443.5,516) -- cycle ;

        % Text Node
        \draw (329,62) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Start of the Run};
        % Text Node
        \draw (329,138) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Episode Phase};
        % Text Node
        \draw (330.38,208.75) node   [align=left] {Undo?};
        % Text Node
        \draw (169,137) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Undo Phase};
        % Text Node
        \draw (331.38,289.75) node  [font=\footnotesize] [align=left] {Is it time to learn?};
        % Text Node
        \draw (169,332) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Training Phase};
        % Text Node
        \draw (245,200) node  [font=\scriptsize] [align=left] {Yes};
        % Text Node
        \draw (346,243) node  [font=\scriptsize] [align=left] {No};
        % Text Node
        \draw (234,278) node  [font=\scriptsize] [align=left] {Yes};
        % Text Node
        \draw (346,326) node  [font=\scriptsize] [align=left] {No};
        % Text Node
        \draw (331.38,392.75) node  [font=\footnotesize] [align=left] {Is it time to test?};
        % Text Node
        \draw (511,436) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Testing Phase};
        % Text Node
        \draw (425,384) node  [font=\scriptsize] [align=left] {Yes};
        % Text Node
        \draw (346,428) node  [font=\scriptsize] [align=left] {No};
        % Text Node
        \draw (511,496) node  [color={rgb, 255:red, 255; green, 255; blue, 255 }  ,opacity=1 ] [align=left] {Checkpoint Phase};
