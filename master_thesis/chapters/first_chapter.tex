...
\begin{figure}
	...
	% node r_t
	\node[mylabel, below left=0.75cm and -0.2cm of agent] (reward) {$r_t$};
	% node s_t+1
	\node[mylabel, above left=-1.2cm and -1cm of env] (state) {$s_{t+1}$};
	% node r_t+1
	\node[mylabel,above left=-.45cm and -1cm of env] (reward1) {$r_{t+1}$};
	
	
	
	\draw[pil]   (agent.east) -- ($(agent.east) + (1.2cm,0cm)$)  |-  (env.east);
	\draw[pil]   ($(env.west) + (0,-0.1cm)$) -- ($(env.west) + (-1.2cm,-0.1cm)$);
	\draw[pil]   ($(env.west) + (-1.2cm,-0.1cm)$) -- ($(env.west) + (-2cm,-0.1cm)$) |-($(agent.west) + (0,0.1cm)$);
	\draw[pil]   ($(env.west) + (0,+0.1cm)$) -- ($(env.west) + (-1.2cm,+0.1cm)$);
	\draw[pil]   ($(env.west) + (-1.2cm,+0.1cm)$) -- ($(env.west) + (-1.8cm,+0.1cm)$) |-($(agent.west) + (0,-0.1cm)$);
...
\todomacaluso{A \textit{model} is composed by information about the environment. These data must not be confused with \textit{states} and \textit{observations}: they make it possible to infer prior knowledge about the environment, influencing the behaviour of the agent.}
